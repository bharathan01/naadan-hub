...
export default function LoginPage() {
  ...
  // Reactive redirection based on global auth state
  useEffect(() => {
    if (user && role) {
      if (role === 'admin') {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Sign in requested...');
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="min-h-[calc(100vh-80px)] bg-gradient-to-br from-green-50 via-yellow-50 to-orange-50 flex items-center justify-center px-4 py-12">
        <motion.div
          ...
          <div className="bg-white rounded-3xl shadow-xl p-8">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-primary text-white py-3 rounded-xl font-semibold hover:bg-primary/90 transition-colors cursor-pointer whitespace-nowrap disabled:opacity-50"
              >
